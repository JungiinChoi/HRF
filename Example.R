...
h4 = NLgamma_fitted$hrf[,1]
fit4 = NLgamma_fitted$fit
e4 = NLgamma_fitted$e
...
ggplot(HRF_df) +
  ...
  geom_line(aes(x=t, y=fit2, color = 'sFIR')) +
  geom_line(aes(x=t, y=fit3, color = 'DD')) +
  geom_line(aes(x=t, y=fit, color = 'NL')) +
  theme_minimal() +
  labs(title = "Sample time course", x = "", y = "") +
  geom_segment(aes(x = onset, y = -4, xend = onset, yend = -3)) +
  coord_cartesian(xlim = c(0, 640)) +
  scale_color_manual(name = "", values=c('blue', 'magenta', 'red', 'yellow', 'green'))
